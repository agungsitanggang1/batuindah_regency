...
class Blok extends CI_Controller {
	...
	public function insert()
	{
		...
		$object = array(
			'nama_blok'=> 'Blok '.strtoupper($nama_blok),
			'is_aktif'=>$is_aktif
		);
		...
		}else if($status== 'waduh'){
			$message = array('message'=>'Blok tidak boleh sama', 'class'=>'alert alert-danger fade in');
			$this->session->set_flashdata('item', $message);			
		}else{
	...
	{		
		...
		$data['lihat_rumah2'] 	= $this->db->get_where('rumah',array('id_blok'=>$id))->result_object();
		
		$this->db->select('*');
		$this->db->join('penghuni','penghuni.id_rumah = rumah.id_rumah');
		$this->db->where('id_blok',$id);
		$data['lihat_rumah'] = $this->db->get('rumah')->result();		
	...
	public function update()
	{
		...
		$message = array('message'=>'Data gagal diubah', 'class'=>'alert alert-danger fade in');
		$this->session->set_flashdata('item', $message);			
				redirect('blok');
		}
