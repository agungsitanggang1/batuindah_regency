...
class Surat extends CI_Controller {
	...
	public function daftarPengajuan(){
		...
		$data['menu'] 			= 'pengajuan_surat';
	 	$data['data'] = $this->db->query("SELECT * FROM pengajuan_surat ORDER BY tanggal_surat DESC")->result();
	...
	public function hapus($id)
	{
		...
		$hapus = $this->SuratM->delete($id);
		
		if($this->db->affected_rows()){
			$message = array('message'=>'Data Surat berhasil dihapus', 'class'=>'alert alert-success fade in');
			$this->session->set_flashdata('item', $message);
		}else{
			$message = array('message'=>'Data Surat gagal dihapus', 'class'=>'alert alert-error fade in');
			$this->session->set_flashdata('item', $message);			
		}
		$this->session->keep_flashdata('item',$message);
		redirect('Surat/daftarPengajuan');
	}
