...
class UserPemakai extends CI_Controller {
	...
	public function insert()
	{
		...
		$is_aktif  = true;
		$idpeng = $this->input->post('id_penghuni');
		$object = array(
			'id_role'=> $id_role,
			'username'=> $username,
			'password'=> $password,
		...
		if($status == 'berhasil'){
			...
			$message = array('message'=>'Penghuni Tidak Dipilih', 'class'=>'alert alert-danger fade in');
			$this->session->set_flashdata('item', $message);
	...
	public function update()
	{
		...
		if($id_role == '1'){
			...
			$rt = $this->db->query("SELECT id_user_pemakai FROM user_pemakai WHERE id_role ='1' ORDER BY id_role DESC LIMIT 1")->row('id_user_pemakai');

			$rto = array(
			'id_role'=>'3'
			);

			$this->db->where('id_user_pemakai', $rt);
			$this->db->update('user_pemakai', $rto);
			...
			$rw = $this->db->query("SELECT id_user_pemakai FROM user_pemakai WHERE id_role ='2' ORDER BY id_Role DESC LIMIT 1")->row('id_user_pemakai');
			$rwo = array(
			'id_role'=>'3'
			);

			$this->db->where('id_user_pemakai', $rw);
			$this->db->update('user_pemakai', $rwo);
